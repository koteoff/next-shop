...
export const ChooseProcForm: React.FC<Props> = ({
  ...
  const {
    ...
    setType,
    addIngredient,
  } = useProcOptions(items);

  const totalAddServicesPrice = addservices
    .filter((addservice) => selectedIngredients.has(addservice.id))
    .reduce((acc, addservice) => acc + addservice.price, 0);
  const totalPrice =
    items.find((item) => item.BoxType === type)?.price ||
    0 + totalAddServicesPrice;
          ...
          <div className="grid grid-cols-3 gap-3">
            {addservices.map((addservice) => (
              <AddServicesItem
                ...
                active={selectedIngredients.has(addservice.id)}
...
// Хук usePizzaOptions
export const useProcOptions = (items: ProductItem[]): ReturnProps => {
